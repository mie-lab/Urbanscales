...
# Load the CSV files
df_scale_25 = pd.read_csv('./all_day_all_cities_scale_25.csv', sep="\t")
df_scale_50 = pd.read_csv('./all_day_all_cities_scale_50.csv', sep="\t")
df_scale_100 = pd.read_csv('./all_day_all_cities_scale_100.csv', sep="\t")
...
plt.savefig("All_day_all_cities_GoF_LR")
plt.show()
...
plt.ylim(0, 1)
plt.xlabel('Area of each urban tile')
plt.xticks(ticks=[0, 1, 2], labels=[r'4 $km^2$', r'1 $km^2$', r'0.25 $km^2$'], rotation=0)  # Change x-axis labels
plt.savefig("All_day_all_cities_GoF_RF")
plt.show()
